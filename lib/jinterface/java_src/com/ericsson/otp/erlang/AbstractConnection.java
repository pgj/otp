...
public abstract class AbstractConnection extends Thread {
    ...
    protected void sendMonitorExit(final OtpErlangPid from, final OtpErlangPid dest,
    ...
    public void run() {
        ...
        try {
            receive_loop: while (!done) {
                ...
                case monitorTag: // { MONITOR_P, FromPid, ToPid, Ref }
                    if (head.elementAt(3) == null) {
                        break receive_loop;
                    }
                    if (traceLevel >= ctrlThreshold) {
                ...
                case demonitorTag: // { DEMONITOR_P, FromPid, ToPid, Ref }
                    if (head.elementAt(3) == null) {
                        break receive_loop;
                    }
                    if (traceLevel >= ctrlThreshold) {
                ...
                case monitorExitTag: // { MONITOR_P_EXIT, FromPid, ToPid, Ref, Reason }
                    if (head.elementAt(4) == null) {
                        break receive_loop;
                    }
                    if (traceLevel >= ctrlThreshold) {
